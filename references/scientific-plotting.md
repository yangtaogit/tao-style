...
- Matplotlib: put the Z axis line, ticks, tick labels and title on the screen-left side by default. `apply_matplotlib_3d_style(..., zaxis_side="left")` selects the side at the current view; set `view_init` first. After changing the view, call `set_matplotlib_3d_zaxis_side(ax, "left")` again before the box-edge and colorbar helpers. Use `zaxis_side="default"` to request native placement, or `"right"` for screen-right.
- Use `save_adaptive_figure` for 3D exports: it includes visible 3D axis titles and offset text in the tight bounds, so the left Z title is not cropped. For direct `fig.savefig`, include those artists with `bbox_extra_artists`. The side helper uses native lower/upper position controls on Matplotlib 3.8+ and per-axis edge ordering on older versions; verify the final view, especially after nonzero roll or changing the vertical axis.
