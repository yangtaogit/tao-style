...
- Matplotlib 3D Z axis: place the axis line, ticks, tick labels and Z title on the screen-left side by default, keeping right-side colorbars outside the plot. Set the view before styling and use the adaptive save helper to include the left-side title. This preference does not change Plotly native 3D rendering.
- 3D axes: use orthographic projection and a transparent black wireframe coordinate box by default. Preserve equal physical scale when X/Y/Z use comparable units, keep colorbars outside plotted content, and use hidden axes only when Tao asks. Follow `references/scientific-plotting.md` for the complete 3D rules and helper call sequence, including `add_matplotlib_3d_box_edge(ax, fig)`.
...
Use the resolved `<skill-root>/scripts/apply_tao_style.py` as the Matplotlib implementation of the current scientific plotting profile. Update that script whenever confirmed reusable Matplotlib defaults change.
