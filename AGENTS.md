...
- Optional tao palette exists for special cases; it should not replace the default cool palette unless Tao asks.- Colorbar: outside right, vertical, black outline matching the axes box; portrait figures with right-side colorbars should expand canvas width rather than overlap or shrink the axes box.
- 3D plots: use orthographic projection `projection="ortho"` by default (Z stays vertical, heights comparable); perspective only for presentation-style depth; pane color `#F2F2F2`; major-tick grid only with color `#9E9E9E`, dotted linestyle `":"`, and linewidth `0.2 pt`; no extra pane boundary lines or manual frames; inward ticks using `inward_factor=0.0` and `outward_factor=0.2`; reclaim space in layers: `set_box_aspect(None, zoom=1.2)` plus content-adaptive cropping, about five major ticks per axis with unit-scaled short tick text, then mild `tick_pad=-1.5` and `labelpad=-3.0`, with a tighter `zlabelpad=-6.0` for the Z title because Z tick numbers are horizontal text whose width sets the title offset (starting values, verify at the final view angle); right-side colorbars positioned from the axes' tight bounding box, not the axes rectangle, so they never overlap 3D tick or axis labels (helper `add_matplotlib_3d_colorbar`, default `pad=0.28 in`, `shrink=0.72`, expands canvas rightward when needed; add after content, view angle, and box aspect are final).
